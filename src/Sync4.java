...
// ** Same as Sync3, but with support for -bundle ** //

// Work\workspace: 363MB, 4772 files
//   bundle: 420 secs, 49MB
//   files:  820 secs, 363MB
...
public class Sync4 {
	...
	public void parseArgs(String[] args) throws IOException {
		for(int i=0; i<args.length; i++) {
			if(args[i].equalsIgnoreCase("-nodel")) {
				...
				this.skips.add((File)new File(this.source_dir, args[++i]).getCanonicalFile());
			}
				...
				if(this.source_dir == null)
					this.source_dir = (File)new File(args[i], ArchiveDetector.NULL).getCanonicalFile();
				else if(this.dest_dir == null)
					this.dest_dir = (File)new File(args[i], ArchiveDetector.NULL).getCanonicalFile();
				else
	...
	/**
	 *  Copy and remove files as needed in this one dir; ignore sub-dirs.
	 *  Ignores skips; it's assumed caller has filtered out this dir.
	 *  Handling of excludes is not implemented.
	 */
	private void syncFiles(File source_dir) {
		...
		// WARNING:
		// Do not pass this ddst around to other functions, unless you're CERTAIN
		// it's safe!  This will interpret zip files as dirs, then try to write
		// inside them.  We MUST NOT mangle user zips, only this special bundle zip.
		File ddst = new File(this.dest_dir, FileUtils.relativize(this.source_dir, dsrc));
		if(bundle)
			ddst = new File(ddst, "__BasementSync_bundle.zip", ArchiveDetector.DEFAULT);
			...
			for(Iterator<String> fi = srcfiles.iterator(); fi.hasNext(); ) {
				...
				if(needsSync(fsrc, fdst)) {
					...
					try {
						...
						if( !fsrc.archiveCopyTo(fdst) )
							throw new IOException("archiveCopyTo failed");
					}
					catch(IOException ex) {
